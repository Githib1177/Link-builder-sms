import { fingerprint, parseResponse } from './sms-monitor.js';
...
export function sendQuery({login,password,number,message,id}) {
  // Preserve the existing account-selected sender for ALL recipients.
  // Do not add sender_id or sender_phone without a separately approved guest sender.
  return new URLSearchParams({action:'send_sms',login,password,number,message,delivery_report:'1',user_id:id.slice(0,50)});
}
